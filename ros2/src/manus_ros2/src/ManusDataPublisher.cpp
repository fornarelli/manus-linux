// PORTED TO MANUS SDK 2.5.1
// 3.x renamed the thumb ergonomics enums to anatomically correct joints;
// they address the same four slots of ErgonomicsData.data, so this is a
// pure rename (verified against SDKClient 2.5.1's PrintHandErgoData, which
// labels those slots cmc/cmc/mcp/ip):
//   CMCSpread->MCPSpread  CMCStretch->MCPStretch
//   MCPStretch->PIPStretch  IPStretch->DIPStretch
...
ManusDataPublisher::ManusDataPublisher() : Node("manus_data_publisher")
{
    ...
    // PORT NOTE: upstream hardcodes integrated mode. Integrated needs a licence
    // with the Integrated feature; a legacy licence (e.g. Core XR) only covers
    // remote, so the mode has to be selectable. Default differs from upstream.
    //   connection_mode: "remote" (default) | "local" | "integrated"
    //   core_ip:         pin a specific MANUS Core; empty = autodiscover
    const std::string t_Mode = declare_parameter<std::string>("connection_mode", "remote");
    m_Ip = declare_parameter<std::string>("core_ip", "");
    if (t_Mode == "integrated")   m_ConnectionType = ConnectionType::ConnectionType_Integrated;
    else if (t_Mode == "local")   m_ConnectionType = ConnectionType::ConnectionType_Local;
    else if (t_Mode == "remote")  m_ConnectionType = ConnectionType::ConnectionType_Remote;
    else
    {
        ClientLog::error("Unknown connection_mode '{}' - use integrated, local or remote.", t_Mode);
        throw std::runtime_error("Unknown connection_mode");
    }
